...
#ifndef MXB_POOL_NUM_THREADS
	#define MXB_POOL_NUM_THREADS 0
#endif

#ifndef MXB_POOL_NUM_BUFFERS
	#define MXB_POOL_NUM_BUFFERS 10
#endif
#ifndef MXB_POOL_STACK_SIZE
	#define MXB_POOL_STACK_SIZE 0
#endif

#ifndef MXBS_DEFAULT_THREADSPERPROC
	#define MXBS_DEFAULT_THREADSPERPROC 2
#endif

#ifndef MXBS_DEFAULT_THREADPOOLSHUTDOWNTIMEOUT
	#define MXBS_DEFAULT_THREADPOOLSHUTDOWNTIMEOUT 36000
#endif
...
typedef Blxware::SyncQueue< WSAOVERLAPPED* > QEventData;
typedef QEventData* PQEventData;
...
#define MXBS_POOL_SHUTDOWN ((OVERLAPPED*) ((__int64) -1))
...
public:
	...
	CThreadPool() throw() :
		...
		m_dwMaxWait(MXBS_DEFAULT_THREADPOOLSHUTDOWNTIMEOUT),
		m_bShutdown(FALSE),
		m_dwThreadEventId(0),
		m_dwStackSize(0)
    {
        m_pQEventData = new QEventData( MXB_POOL_NUM_BUFFERS );
...
#pragma warning(pop)
	...
	{
		if (nNumThreads == 0)
			nNumThreads = -MXBS_DEFAULT_THREADSPERPROC;
	...
	BOOL QueueRequest(_In_ typename Worker::RequestType request) throw()
	{
        ...
        if( !m_pQEventData->push_timed( request, 5000 ) ) 
        {
            return FALSE;
        }
...
protected:
	...
	DWORD ThreadProc() throw()
	{
		...
		// thread handle is closed
		{
			...
			{
				if (pOverlapped == MXBS_POOL_SHUTDOWN) // Shut down
				{
	...
	_Check_return_ HRESULT InternalResizePool(_In_ int nNumThreads, _In_ int dwMaxWait) throw()
	{
		...
		else if (nNumThreads < nCurThreads)
		{
			...
			for (int nThreadIndex = 0; nThreadIndex < nNumShutdownThreads; nThreadIndex++)
			{
				...
				PostQueuedCompletionStatus(m_hRequestQueue, 0, 0, MXBS_POOL_SHUTDOWN);
