...
osIPC::Server::Server(PCHAR QueueName)
{
	// Set default params
	m_hMapFile = 0;
	m_hSignal = 0;
...
void osIPC::Server::create(PCHAR QueueName)
{
...
DWORD osIPC::Server::readRef(void *pBuff, DWORD buffSize, DWORD dwTimeout)
{
	// Grab a block
	Block *pBlock = getBlock(dwTimeout);
	if (!pBlock) return 0;
...
DWORD osIPC::Client::writeRef(void *pBuff, DWORD amount, DWORD dwTimeout)
{
	// Grab a block
	Block *pBlock = getBlock(dwTimeout);
	if (!pBlock) return 0;
